...
stacked_barchart <- function(data, chartcolors, ylabel = "Visit") {
  ...
  fig <- ggplot(df_stacked, aes(x = percentage, y = visit, fill = brcat)) +
    ...
    scale_fill_manual(values = chartcolors, breaks = rev(levels(df_stacked$brcat))) +
...
#' @param chartcolors `vector` a vector of colors, the same number of levels as
#' the brcat variable
#' @param favcat `vector` a vector of favorable categories in the desired plot order
#' @param unfavcat `vector` a vector of unfavorable categories in the desired plot order
#' @param ylabel `character` y label name, default is "Visit"
#'
#' @return a ggplot object
#' @export
...
#'   data = comp_outcome,
#'   chartcolors = colfun()$fig12_colors,
#'   favcat = c("Benefit larger than threshold, with AE", "Benefit larger than threshold, w/o AE"),
#'   unfavcat = c("Withdrew", "Benefit less than threshold, w/o AE", "Benefit less than threshold, with AE"),
#'   ylabel = "Study Week"
#' )
#'
divergent_stacked_barchart <- function(data, chartcolors, favcat, unfavcat, ylabel = "Visit") {
  all_columns <- c(
    "usubjid", "visit", "trt", "brcat"
  )
  nonexistent_columns <- setdiff(all_columns, colnames(data))
  ...
  fig <- ggplot(df_stacked_new, aes(x = percentage, y = visit, fill = brcat)) +
    ...
    geom_text(aes(label = ifelse(side == "Left", -percentage, percentage)),
              ...
              position = position_stack(vjust = 0.5),
              size = control_fonts()$p * 0.45) +
    scale_x_continuous(breaks = seq(-100, 100, 20), limits = c(-100, 100), expand = c(0, 0), labels = function(breaks) ifelse(breaks > 0, breaks, abs(breaks))) +
    xlab("Percentage") +
    ylab(ylabel) +
    guides(fill = guide_legend(title = "Outcome", nrow = 3, byrow = TRUE)) +
    labs(color = NULL) +
