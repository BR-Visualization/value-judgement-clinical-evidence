...
This document explains how to set up a pre-commit hook to run lintr checks locally before code reaches GitHub CI/CD.

## Why Use Pre-commit Hooks?

Pre-commit hooks catch linting issues **before** you commit, preventing failed CI/CD builds and reducing the feedback loop.
...
The hook file is already created at `.git/hooks/pre-commit`. On Windows, you may need to ensure Git can execute it.

**On Windows:**
- Git Bash should work automatically
- If you have issues, you can use the PowerShell version instead
...
The pre-commit hook runs `lintr::lint_package()` before each commit. If any linting issues are found:
- ❌ The commit is blocked
- Issues are displayed in the console
- You can fix the issues and try committing again
...
1. **`pre-commit`** (default) - Runs `lintr::lint_package()` on the entire package
   - More thorough
...
2. **`pre-commit-staged-only`** - Only lints files being committed
   - Faster
   - Less comprehensive
   - To use: `mv .git/hooks/pre-commit-staged-only .git/hooks/pre-commit`

## Troubleshooting

### Hook not running
- Verify the file is named exactly `pre-commit` (no extension) in `.git/hooks/`
...
3. Note: `.git/hooks/` is not tracked by Git, so each developer must set it up locally

Alternatively, consider using the [`pre-commit` framework](https://pre-commit.com/) for team-wide hook management.
